...
class QnpReducer:
    def __init__(self,ff_solver,qnp_path,output_dir,clean=False) -> None:
        ...
        with open(qnp_path,'r') as f:
            self.qnp.loadQnp(f)        
        qnp_name = qnp_path.split("/")[-1].split('.')[0]
        self.clean_up = False
        if output_dir == "":
    ...
    def detect_redunant(self)->list:
        ...
        for action in (self.qnp.actions):
            if action not in actions_seq:
                redunant_actions.append(action)
                if len(redunant_actions) <= 10:
                    print(action)
                if len(redunant_actions) == 11:
                    print("...")
        print(f"Redunant: {len(redunant_actions)} actions")
        return redunant_actions
        ...
    
    def reduce_and_dump(self,std_out = False):
        redunant_actions = self.detect_redunant()
        reduced_qnp = QnpParser()
        with open(self.qnp_path,'r') as f:
            reduced_qnp.loadQnp(f)
        for redunant_action in redunant_actions:
            reduced_qnp.actions.pop(redunant_action)
        reduced_pddler = QnpPddl(reduced_qnp)
        with open(os.path.join(self.output_dir,"redundant_actions"),'w') as f:
            f.write("\n".join(redunant_actions))
...
if __name__ == "__main__":
    ...
    args_parser.add_argument('--ff_solver',
        help='version of ff solver, default is ff_3k',default="ff_3k")
