...
class SASTask:
    ...
    def output(self, stream):
        ...
        for axiom in self.axioms:
            axiom.output(stream)
        print(len(self.fairnesses), file=stream)
        for fairness in self.fairnesses:
            fairness.output(stream)
...
class SASFairness:
    def __init__(self, fairness):
        # Note we only have to process the OR_action currently.
        # self.fairness = [fairness_a,fairness_b]
        self.fa, self.fb = fairness
        
    def output(self, stream):
        print("begin_fairness", file=stream)
        print(len(self.fa), len(self.fb), file=stream)
        print("\n".join(self.fa + self.fb), file=stream)
        print("end_fairness", file=stream)
