...
class VarValueRenaming(object):
    ...
    def apply_to_task(self, task):
        ...
        removed_ops = set()
        self.apply_to_operators(task.operators, removed_ops)
        self.apply_to_fairnesses(task.fairnesses, removed_ops)
    ...
    def apply_to_operators(self, operators, removed_ops):
        new_operators = []
        num_removed = 0
        for op in operators:
            try:
                ...
                num_removed += 1
                removed_ops.add(op.name)
                if DEBUG:
                    print("Removed operator: %s" % op.name)
        print("%d operators removed" % num_removed)
        operators[:] = new_operators

    def apply_to_fairnesses(self, fairnesses, removed_ops):
        for fairness in fairnesses:
            fairness.fa[:] = list(filter(lambda x: x not in removed_ops, fairness.fa))  
            fairness.fb[:] = list(filter(lambda x: x not in removed_ops, fairness.fb))  
