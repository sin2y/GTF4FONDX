...
def translate_strips_operator(operator, dictionary, ranges, mutex_dict,
    ...
    if len(conditions) > 1:
        suffixes = [(f"{global_renaming.SUFFIX_MULTI_CONDITIONS}%d" % (i+1)) for i in range(len(conditions))]
    else:
        suffixes = ['']
...
def translate_strips_operator_aux(operator, dictionary, ranges, mutex_dict,
    ...
    new_name = ' '.join([operator.name.split(' ')[0] + suffix] + operator.name.split(' ')[1:])
...
def translate_task(strips_to_sas, ranges, translation_key,
                   ...
                   init, goals,
                   actions, axioms, fairness_pairs, metric, implied_facts):
    ...
    # Build Fairness
    fairnesses = []
    map_action_operaters = defaultdict(set)
    for op in operators:
        raw_name = global_renaming.get_raw_name(op.name)
        map_action_operaters[raw_name].add(global_renaming.get_nondet_name(op.name))

    def refine_action(action):
        if not map_action_operaters[action]:
            print(f"\033[91mWarning! Action {action} is not detected in the sas plan!!!\033[0m")
            return []
        else:
            return (list(map_action_operaters[action]))

    for fa,fb in fairness_pairs:
        new_fa = list(itertools.chain.from_iterable(refine_action(action) for action in fa))
        new_fa.sort()
        new_fb = list(itertools.chain.from_iterable(refine_action(action) for action in fb))
        new_fb.sort()
        if(new_fa or new_fb):
            fairnesses.append(sas_tasks.SASFairness((new_fa,new_fb)))        

    return sas_tasks.SASTask(variables, mutexes, init, goal,
                             operators, axioms, fairnesses, metric)
...
def pddl_to_sas(task):
    ...
    with timers.timing("Translating task", block=True):
        sas_task = translate_task(
            ...
            mutex_dict, mutex_ranges, mutex_key,
            task.init, goal_list, actions, axioms, task.fairnesses, task.use_min_cost_metric,
            implied_facts)
