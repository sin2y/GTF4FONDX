...
class Action(object):
    ...
    def parse(alist):
        ...
        try:
            ...
            if 1 == len(cost_eff_pairs):
                ...
                cost_eff_pairs = [(cost_eff_pairs[i][0], cost_eff_pairs[i][1], f"{global_renaming.SUFFIX_DETERMINIZATION}%d" % i) for i in range(len(cost_eff_pairs))]
    ...
    def instantiate(self, var_mapping, init_facts, fluent_facts, objects_by_type):
        ...
        name = "%s" % (" ".join([self.name] + arg_list))
