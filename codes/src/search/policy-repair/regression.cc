...
string RegressionStep::get_name() {
    if (is_goal)
        ...
        return op->get_nondet_name() + " / d=" + static_cast<ostringstream*>( &(ostringstream() << distance) )->str();
}

void NondetDeadend::dump() const {
    cout << "Non-deterministic deadend:" << endl;
    cout << "Operator: " << g_operators[op_index].get_name() << endl;
    cout << "Non Operator: " << (*g_nondet_mapping[op_index])[0]->get_name() << endl;
    cout << " -{ State }-" << endl;
    state->dump_pddl();
    cout << "" << endl;
}
...
list<std::shared_ptr<PolicyItem>> perform_regression(const SearchEngine::Plan &plan, std::shared_ptr<RegressionStep> goal_step, int distance, bool create_goal) {
    g_timer_regression.resume();
    list<std::shared_ptr<PolicyItem>> reg_steps;
    std::shared_ptr<PartialState> s = make_shared<PartialState> (g_initial_state());

    vector<std::shared_ptr<PartialState> > states;
    states.push_back(s);

    for (int i = 0; i < plan.size(); i++) {
        s = make_shared<PartialState> (*s, *plan[i]);
        states.push_back(s);
    }
    ...
    if (g_fullstate) {
        ...
        if (create_goal) {
            ...
            std::shared_ptr<PartialState> g = make_shared<PartialState> (*(goal_step->state));

            reg_steps.push_back(make_shared<RegressionStep>(g, distance));

        } else {
            reg_steps.push_back(make_shared<RegressionStep>(states.back(), distance));
        }
        ...
        for (int i = plan.size() - 1; i >= 0; i--) {
            std::shared_ptr<RegressionStep> next = std::static_pointer_cast<RegressionStep>(reg_steps.back());
            reg_steps.push_back(make_shared<RegressionStep>(*plan[i], states.back(), ++distance, next));
            next->prev = std::static_pointer_cast<RegressionStep>(reg_steps.back());
            states.pop_back();
        }
        ...
        s = make_shared<PartialState> (*(goal_step->state));

        reg_steps.push_back(make_shared<RegressionStep>(s, distance));

        for (int i = plan.size() - 1; i >= 0; i--) {

            std::shared_ptr<RegressionStep> next = std::static_pointer_cast<RegressionStep>(reg_steps.back());

            std::shared_ptr<PartialState> regressed = make_shared<PartialState> (*(next->state), *plan[i], false, states[i]);

            reg_steps.push_back(make_shared<RegressionStep>(*plan[i], regressed, ++distance, next));

            next->prev = std::static_pointer_cast<RegressionStep>(reg_steps.back());

            // Strengthen the step so it doesn't fire an FSAP at some
            // some point. Strengthening is sufficient but not neccessary
            ((RegressionStep*)reg_steps.back().get())->strengthen(states[i]);
//            (std::static_pointer_cast<RegressionStep>(reg_steps.back()))->strengthen(states[i]);
...
//        delete reg_steps.front();
        reg_steps.pop_front();
        ((RegressionStep*)reg_steps.front().get())->next = goal_step;
    }
...
//    if(g_findsc_info){
//        for (list<std::shared_ptr<PolicyItem>>::iterator op_iter = reg_steps.begin(); op_iter != reg_steps.end(); ++op_iter) {
...
void generate_regressable_ops() {
    ...
    list<std::shared_ptr<PolicyItem>> reg_steps;
    list<std::shared_ptr<PolicyItem>> cond_reg_steps;
    ...
    for (int i = 0; i < g_operators.size(); i++) {
        ...
        if (0 == g_nondet_conditional_mask[g_operators[i].nondet_index]->size()) {
            ...
            reg_steps.push_back(make_shared<RegressableOperator>(g_operators[i], s));
        }
        ...
        // Next, consider operators that have conditional effects that are consistent
            ...
            if (consistent)
                cond_reg_steps.push_back(make_shared<RegressableOperator>(g_operators[i], s));
        }
    }
...
void RegressionStep::strengthen(std::shared_ptr<PartialState> s) {

    if (is_goal)
        return;

    vector<std::shared_ptr<PolicyItem>> reg_items;
    ...
    for (int i = 0; i < reg_items.size(); i++) {
        ...
        if (((NondetDeadend*)(reg_items[i]).get())->op_index == op->nondet_index) {

            for (int j = 0; j < g_variable_name.size(); j++) {

                int val = (*(((NondetDeadend*)(reg_items[i].get()))->state))[j];
