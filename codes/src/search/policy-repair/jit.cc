...
bool perform_jit_repairs(Simulator *sim) {
    ...
    std::shared_ptr<PartialState>  current_state; // The current step in the loop
    std::shared_ptr<PartialState>  previous_state; // The previous step in the loop (leading to the current_state)
    std::shared_ptr<PartialState>  current_goal; // The current goal in the loop
    const Operator * prev_op; // The last operator used to get us to current_state
    std::shared_ptr<RegressionStep>  prev_regstep; // The last RegressionStep that triggered prev_op
    ...
    bool debug_jic = g_running_debug && g_findsc_info;//true;//false;
    ...
    std::shared_ptr<PartialState> old_initial_state = make_shared<PartialState> (g_initial_state());

    // Build the goal state
    std::shared_ptr<PartialState> goal_orig = make_shared<PartialState> ();
    ...
    if (g_policy->seen) {
        ...
        created_states = new vector<std::shared_ptr<PartialState> >();

        current_state = make_shared<PartialState> (g_initial_state());
        current_goal = make_shared<PartialState> (*goal_orig);
    ...
    while (!open_list->empty() /*&& ((g_timer_jit() < g_jic_limit) || g_record_relevant_pairs)*/) {
        ...
        if (debug_jic) {
            cout << "\n\nChecking state:" << endl;
            current_state->dump_pddl(); current_state->dump_fdr();
            cout << "===================" << endl;
            cout << "\n\nCurrent Goal:" << endl;
            current_goal->dump_pddl(); current_goal->dump_fdr();
            cout << "===================" << endl;
        ...
        if (0 == seen->count(*current_state)) {
            ...
            std::shared_ptr<RegressionStep>  regstep = g_policy->get_best_step(*current_state);
            ...
            if (0 == regstep) {
                ...
                if (g_record_relevant_pairs) {
                    ...
                    if (g_optimized_scd && prev_regstep && have_solution) {
                        ...
                        std::shared_ptr<PartialState>  prev_str_state = make_shared<PartialState> (*(regstep->state), *prev_op, false, prev_regstep->state);
                        std::shared_ptr<RegressionStep> prev_str_regstep = prev_regstep;
                        std::shared_ptr<RegressionStep> str_regstep = regstep;
                        std::shared_ptr<PartialState>  updated = NULL;
                        ...
                        while(repeating) {
                            ...
                            for (int i = 0; i < g_variable_name.size(); i++) {
                                    ...
                                    (-1 == (*(prev_str_regstep->state))[i]))
                                {
                                    strengthened = true;
                                    if (!updated)
                                        updated = make_shared<PartialState> (*(prev_str_regstep->state));
                            ...
                            if (updated) {
                                ...
                                str_regstep = make_shared<RegressionStep>(*(prev_str_regstep->op), updated, prev_str_regstep->distance, str_regstep, prev_str_regstep->prev);
                                ...
                                if(debug_jic){
                                    str_regstep->state->dump_pddl();
                                    prev_str_regstep->op->dump();
                                    prev_str_regstep->state->dump_pddl();
                                }

                                if (prev_str_regstep)
                                    prev_str_state = make_shared<PartialState> (*(str_regstep->state), *(prev_str_regstep->op), false, prev_str_regstep->state);
            ...
            if (have_solution) {
                ...
                if ( ! (regstep->is_goal || (g_optimized_scd && regstep->is_sc))) {
                    ...
                    std::shared_ptr<PartialState> full_expected_state = make_shared<PartialState> (*current_state, *(regstep->op));
                    std::shared_ptr<PartialState> expected_state = full_expected_state;
                    created_states->push_back(full_expected_state);

                    std::shared_ptr<RegressionStep>  expected_regstep = g_policy->get_best_step(*full_expected_state);

                    if (g_partial_planlocal && expected_regstep) {
                        expected_state = make_shared<PartialState> (*(expected_regstep->state));
                    ...
                    for (int i = 0; i < g_nondet_mapping[regstep->op->nondet_index]->size(); i++) {
                        std::shared_ptr<PartialState> new_state = make_shared<PartialState> (*current_state, *((*(g_nondet_mapping[regstep->op->nondet_index]))[i]));
...
//                    }
                    ...
                    if (g_detect_deadends) {
                        ...
                        if(debug_jic){
                            cout << "dumping failed state: \n==========";
                            cout << "current_state: \n";
                            current_state->dump_pddl();
                            cout << "previous_state: \n";
                            previous_state->dump_pddl();
                            cout << "prev_op: \n";
                            prev_op->dump();
                        }
                    }
    ...
    bool finished_early = false/*(g_jic_limit <= g_timer_jit())*/;
    if (debug_jic) {
...
//        }
    ...
    if (made_change || (g_failed_open_states > 0) || g_updated_deadends || g_policy->is_strong_cyclic()) {
        ...
        if (1/*g_policy->better_than(g_best_policy)*/) {
