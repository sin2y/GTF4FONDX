...
bool is_deadend(PartialState &state) {
    ...
    if(false/*g_running_debug*/){
        cout << "\n\nIs Deadend?" << endl;
        state.dump_pddl();state.dump_fdr();
        cout << "===============" << endl;
    }
...
bool generalize_deadend(PartialState &state) {
    ...
    if(debug){
        cout << "\ngeneralize_deadend(PartialState &state):" << endl;
        state.dump_pddl(); state.dump_fdr();
        cout << "===============" << endl;
    }
...
void update_deadends(vector< DeadendTuple* > &failed_states) {
    ...
    list<std::shared_ptr<PolicyItem>> de_items;
    list<std::shared_ptr<PolicyItem>> de_states;

    bool debug = false;

    std::shared_ptr<PartialState>  dummy_state = make_shared<PartialState> ();

    for (int i = 0; i < failed_states.size(); i++) {

        // Generalize the deadend if need be
        std::shared_ptr<PartialState>  failed_state = failed_states[i]->de_state;
        std::shared_ptr<PartialState>  failed_state_prev = failed_states[i]->prev_state;
        ...
        de_states.push_back(make_shared<NondetDeadend>(make_shared<PartialState> (*failed_state))); // ?
        ...
        for (int j = 0; j < reg_items.size(); j++) {
            ...
            RegressableOperator *ro = (RegressableOperator*)(reg_items[j].get());

            de_items.push_back(make_shared<NondetDeadend>
                               (make_shared<PartialState>
                                (*failed_state, *(ro->op), false, dummy_state),
                                                     ro->op->nondet_index));
            if (debug) {
        ...
        for (int j = 0; j < reg_items.size(); j++) {
            ...
            de_items.push_back(make_shared<NondetDeadend>
                               (make_shared<PartialState>
                                (*failed_state, *(ro->op), false, ro->op->all_fire_context),
                                    ro->op->nondet_index));
        ...
        if (NULL != failed_state_prev) {
            de_items.push_back(make_shared<NondetDeadend>
                               (make_shared<PartialState>
                                (*failed_state, *prev_op, false, failed_state_prev),
    ...
    if (g_repeat_fsap_backwards) {
        for (std::list<std::shared_ptr<PolicyItem>>::iterator it=de_items.begin(); it != de_items.end(); ++it) {
...
void DeadendAwareSuccessorGenerator::generate_applicable_ops(const StateInterface &_curr, vector<const Operator *> &ops) {
    if (g_detect_deadends && g_deadend_policy) {
        ...
        map<int, std::shared_ptr<PolicyItem>> fsap_map;
        ...
        for (int i = 0; i < reg_items.size(); i++) {
            ...
            int index = ((NondetDeadend*)(reg_items[i].get()))->op_index;
        ...
        if (!g_limit_states && g_record_online_deadends &&
            ...
            std::shared_ptr<PartialState> newDE = make_shared<PartialState> ();
            for (int i = 0; i < ruled_out.size(); i++) {
                newDE->combine_with(*(((NondetDeadend*)(fsap_map[ruled_out[i]].get()))->state));
            }
...
bool sample_for_depth1_deadends(const SearchEngine::Plan &plan, std::shared_ptr<PartialState> state) {
    ...
    std::shared_ptr<PartialState>  old_s = state;
    std::shared_ptr<PartialState>  new_s;

    for (int i = 0; i < plan.size(); i++) {
        for (int j = 0; j < g_nondet_mapping[plan[i]->nondet_index]->size(); j++) {
            std::shared_ptr<PartialState> succ_state = make_shared<PartialState> (*old_s, *((*(g_nondet_mapping[plan[i]->nondet_index]))[j]));
            if (is_deadend(*succ_state)) {
                if (g_generalize_deadends)
                    generalize_deadend(*succ_state);
                new_deadends.push_back(new DeadendTuple(succ_state, make_shared<PartialState> (*old_s), (*(g_nondet_mapping[plan[i]->nondet_index]))[j]));
            }
        }

        new_s = make_shared<PartialState> (*old_s, *plan[i]);
        old_s = new_s;
    }
