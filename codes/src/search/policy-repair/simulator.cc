...
Simulator::Simulator(SearchEngine *eng, bool verb) :
...
void Simulator::run() {
    for (int i = 0; i < g_num_trials; i++) {
        ...
        if (g_forgetpolicy) { // Anno:【 false by default 】
            delete g_policy;
            g_policy = new Policy();
...
void Simulator::run_once(bool stop_on_failure, Policy *pol) {
    ...
    std::shared_ptr<PartialState> orig_init_state = make_shared<PartialState> (g_initial_state());
    current_state = make_shared<PartialState> (g_initial_state());
...
bool Simulator::execute_action(const Operator *op) {
    ...
    std::shared_ptr<PartialState> old_state = current_state;
    current_state = make_shared<PartialState> (*old_state, *chosen);
...
bool Simulator::replan() {
    ...
    bool try_again = g_plan_locally; // Will hold later only if the engine was created, and no plan works, and we want to plan locally // Anno 【 True by default 】
    ...
    if (engine_ready) {
        ...
        if (engine->found_solution()) {
            ...
            list<std::shared_ptr<PolicyItem>> regression_steps = perform_regression(engine->get_plan(), g_matched_policy, g_matched_distance, g_policy->empty());
            
            if (verbose)
                cout << "Updating the policy." << endl;
            g_policy->update_policy(regression_steps);

            if(g_findsc_info){ // Anno
                cout << "Dumping the policy and fsaps........................................................" << endl;
                cout << "Updating the policy in replan [found_solution 1st]" << endl;
                g_policy->cout_human_policy();
//                g_policy->cout_human_policy(true);
            }
            ...
            if (try_again) {
                ...
                if (engine->found_solution()) {
                    ...
                    if(g_findsc_info){ // Anno
                        cout << "Dumping the policy and fsaps........................................................" << endl;
                        cout << "Updating the policy in replan [found_solution 2th]" << endl;
                        g_policy->cout_human_policy();
//                        g_policy->cout_human_policy(true);
                    }
