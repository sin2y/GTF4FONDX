...
PartialState::PartialState(const PartialState &predecessor, const Operator &op, bool progress, std::shared_ptr<PartialState> context) {
    assert(!op.is_axiom());
    _allocate();
...
PartialState::PartialState(const Operator &op)
{
    _allocate();
    ...
    for (size_t i = 0; i < op.get_prevail().size(); i++) {
...
bool PartialState::soft_combine_with(const PartialState &other) {
    for (int i = 0; i < g_variable_domain.size(); i++) {
        if((vars[i] == -1) || (other[i] == -1) || (vars[i] == other[i])){
            ;
        }
        else{
            return false;
...
bool PartialState::checkGoal()
{
    for (int i = 0; i < g_goal.size(); i++) {
        if(vars[g_goal[i].first] != g_goal[i].second){
            return false;
        }
    }
    return true;
...
string PartialState::getFactsNames() const {
    string facts_names;
    int real_var_size = g_variable_domain.size();
    if(g_var_action)
        real_var_size -= 1;
    for (int i = 0; i < real_var_size; i++) {
        assert(-1 != vars[i]);

            const string &fact_name = g_fact_names[i][vars[i]];
            facts_names += fact_name + ",";
    }
    return facts_names.substr(0,facts_names.size()-1);
}
...
size_t PartialState::hash() const {
    ...
    if(g_var_action)
        size -= 1;
    return ::hash_number_sequence(vars, size);
}
