...
Operator::Operator(istream &in, bool axiom,int det_index)
    : det_index(det_index){
    marked = false;
    nondet_index = -1;
    ...
    if (!is_an_axiom) {
        ...
        if (name.find("_DETDUP") == string::npos) {
            ...
            std::regex pattern("_DETDUP_\\d+");
            nondet_name = std::regex_replace(name, pattern, "");
        }
    ...
    all_fire_context = make_shared<PartialState> ();
