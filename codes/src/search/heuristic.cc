...
//        if(g_qnp_fsap_policy != 0)
        for (int i = 0; i < reg_items.size(); i++) {
            if(g_findsc_info){
                cout << "Forbidding:" << endl;
                cout << ((NondetDeadend*)(reg_items[i].get()))->get_name() << endl << (((NondetDeadend*)(reg_items[i].get()))->op_index) << endl;
            }
            forbidden_ops.insert(((NondetDeadend*)(reg_items[i].get()))->op_index);
        }
    }
}
...
void Heuristic::evaluate(int, bool,int ) {
    return;
