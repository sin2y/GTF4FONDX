...
bool AdditiveHeuristic::relaxed_exploration(bool include_forbidden) {
    ...
    while (!queue.empty()) {
        ...
        if(false/*g_running_debug*/){
            cout << "Prop:\n" << g_fact_names[prop->var][prop->val];
            if(prop->reached_by){
                int reached_by_op_ind = prop->reached_by->operator_no;
                //            g_operators[reached_by_op_ind].dump();
                cout << " Reached_by-Op-name: "<< g_operators[reached_by_op_ind].get_name() <<" Op-idx: " << reached_by_op_ind << " Non-idx: " << g_operators[reached_by_op_ind].nondet_index;
            }
            cout << "\n distance: " << distance << " prop_cost: " << prop_cost << endl ;
        }
        assert(prop_cost >= 0);
...
//                    (include_forbidden && (unary_op->cost != unary_op->base_cost))
                        // Yuan: Only for strong-cyclic planning do we need to compare the cost!
                     (include_forbidden && (!g_sc_planning || unary_op->cost != unary_op->base_cost))
                    )
