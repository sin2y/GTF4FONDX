...
for i in range(5, 81, 5):
    ...
    problem_str += ')'
    try:
        os.mkdir(f"football_u{istr}")
    except Exception as e:
        print(e)
        pass
    shutil.copy(domain_file,f"football_u{istr}")
    with open(os.path.join(f"football_u{istr}",f"football_u{istr}.pddl"), 'w') as fp:
